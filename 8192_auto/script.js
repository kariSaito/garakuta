...
window.onload = function () {
	...
	game.onload = function () {	//ロード後にこの関数が呼び出される
		...
		kaisuText.font = "28px 游明朝 Demibold";	
		kaisuText.color = fontColor;
		kaisuText.width = 150;
		...
		function checkKakuritsu(){
			...
			else if(nowBunbo == 16384){ kaku16384++; }
			else if(nowBunbo == 32768){ kaku32768++; }
			else if(nowBunbo == 65536){ kaku65536++; }
		}
		...
		function seisekiHyoji(){
			...
			kaku16384Text.text = "1/16384 … " + kaku16384 +"回";
			kaku32768Text.text = "1/32768 … " + kaku32768 +"回";
			kaku65536Text.text = "1/65536 … " + kaku65536 +"回";
			game.popScene();
		...
		let kakuLeftX = 210;
		let kakuLeftY = 95;
		let kakuLeftFont = "24px Meiryo";
		let kakuLeftHaba = 250;
		//1/16384回数テキスト
		const kaku16384Text = new Label();	
		kaku16384Text.font = kakuLeftFont;
		kaku16384Text.color = fontColor;
		kaku16384Text.width = kakuLeftHaba;	
		kaku16384Text.moveTo(kakuLeftX, kakuLeftY);
		seisekiScene.addChild(kaku16384Text);
		//1/32768回数テキスト
		const kaku32768Text = new Label();	
		kaku32768Text.font = kakuLeftFont;
		kaku32768Text.color = fontColor;
		kaku32768Text.width = kakuLeftHaba;	
		kakuLeftY = kakuLeftY + 30;
		kaku32768Text.moveTo(kakuLeftX, kakuLeftY);
		seisekiScene.addChild(kaku32768Text);
		//1/65536回数テキスト
		const kaku65536Text = new Label();	
		kaku65536Text.font = kakuLeftFont;
		kaku65536Text.color = fontColor;
		kaku65536Text.width = kakuLeftHaba;	
		kakuLeftY = kakuLeftY + 30;
		kaku65536Text.moveTo(kakuLeftX, kakuLeftY);
		seisekiScene.addChild(kaku65536Text);
		...
		//開始画面ボタン(ゲームオーバー画面用)	
		const kaisiGEndBtn = new Sprite(120, 42);
		kaisiGEndBtn.moveTo(140, 50);
		kaisiGEndBtn.image = game.assets[kaisiGImg];
		endScene.addChild(kaisiGEndBtn);

		//開始画面ボタンクリック処理
		kaisiGEndBtn.ontouchend = function(){
			game.popScene();
			game.pushScene(startScene);	
		}
